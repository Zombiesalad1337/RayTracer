...
int main() {
	...
	for (int i = 0; i < positions.size(); ++i) {
		// std::cout << "x: " << positions[i].first << ", y: " <<
		// positions[i].second << std::endl;
		int scaled_x = (int)(positions[i].first * 8);
		int scaled_y = (int)(positions[i].second * 8);
		// std::cout << "Scaled_x : " << scaled_x << ", scaled_y: " << scaled_y
		// << std::endl; last check so it doesn't write to canvas[][width]
